...
function baseListRows(baseList) {
  return baseList
    .map((r) => {
      ...
      return `<tr${cls}><td class="market">${marketLabel(r.type)}</td><td class="num">${esc(r.stock_id)}</td><td class="name">${esc(r.stock_name)}</td><td class="num">${fmtNum(r.close_yday, 2)}</td><td class="num">${fmtNum(r.close, 2)}</td><td class="${pctClass(r.change_pct)}">${fmtPct(r.change_pct)}</td><td class="num">${fmtNum(r.volume_lots, 0)}</td><td class="num">${fmtNum(r.volume_avg5_lots, 0)}</td><td class="${pctClass(volChangePct(r))}">${fmtPct1(volChangePct(r))}</td><td class="num">${fmtPct(r.strength)}</td></tr>`;
    })
    .join('\n');
}
